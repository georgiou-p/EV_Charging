...
def find_nearest_charging_station(graph, current_node, planned_route, max_range):
    ...
    """
    print(f"    Looking for charging stations on route within {max_range} hops of node {current_node}")
    
    # Get all stations within range
    all_nearby_stations = []
    
    try:
        distances = nx.single_source_shortest_path_length(graph, current_node, cutoff=max_range)
    ...
    if not all_nearby_stations:
        print(f"    No charging stations found within {max_range} hops!")
        return None
    ...
    for station_node, distance in all_nearby_stations:
        ...
        if station_node in planned_route:
            ...
            # For off-route stations, calculate detour cost
            # Detour cost = distance to station + distance from station back to route
            ...
            for route_node in planned_route:
                try:
                    return_distance = nx.shortest_path_length(graph, station_node, route_node)
            ...
            if closest_route_node:
                ...
                priority = f"DETOUR_{total_detour}"
            else:
    ...
    for i, (station_node, distance, score, priority) in enumerate(scored_stations[:5]):
        print(f"    {i+1}. Node {station_node}: distance={distance}, score={score:.0f}, type={priority}")
...
def calculate_charging_needed(current_soc, battery_range, remaining_distance):
    ...
    Args:
        ...
        battery_range: Maximum nodes possible with full battery
        remaining_distance: Remaining distance to destination in hops
        
    Returns:
        tuple: (needs_charging: bool, current_range: int, deficit: int)
    """
    current_range = int(battery_range * current_soc)
    deficit = remaining_distance - current_range
    needs_charging = deficit > 0
    
    return needs_charging, current_range, max(0, deficit)

def travel_to_charging_station(env, car_id, current_node, charging_node, graph, travel_time_per_hop=1.0):
    """
    Handle travel from current location to a charging station
    ...
    Args:
        ...
        graph: NetworkX graph for pathfinding
        travel_time_per_hop: Time per hop in simulation units
    ...
    # Travel each hop to the charging station
    travel_distance = len(path_to_station) - 1
    if travel_distance > 0:
        total_travel_time = travel_distance * travel_time_per_hop
        yield env.timeout(total_travel_time)
        print(f"[T={env.now:.1f}] Car {car_id}: Arrived at charging station {charging_node} (traveled {travel_distance} hops)")
    else:
        print(f"[T={env.now:.1f}] Car {car_id}: Already at charging station {charging_node}")
    
    return True

def travel_to_next_node(env, car_id, driver, travel_time_per_hop=1.0, battery_range=50):
    """
    Handle travel to the next node in the path
    ...
    yield env.timeout(travel_time_per_hop)
